...
const createCart = async function (req, res) {
    try {
        ...
        if (!validator.isValidObjectId(userId)) {
            res.status(400).send({ status: false, msg: `userId should be valid Id ` })
            return
        }
        const findUser = await userModel.findById({ _id: userId })
        if (!findUser) {
            res.status(400).send({ status: false, alert: `user doesn't exist by this ${userId}` })
            return
        }
        if (!validator.isValidObjectId(productId)) {
            res.status(400).send({ status: false, msg: `productId should be valid Id ` })
            return
        }
        const findProduct = await productModel.findOne({ _id: productId, isDeleted: false });
        // const findProduct = await productModel.findOne({_id: req.body.items[0]['productId'], isDeleted:false});
        if (!findProduct) {
            res.status(400).send({ status: false, alert: `product doesn't exist by this id ${productId}` })
            return
        }
        if (!cartId) {
        ...
        if (cart) {
            ...
            const checkProductInCart = await cartModel.findOne({ _id: cartId, 'items.productId': productId })
            console.log("checkProductInCart", checkProductInCart)
            if (checkProductInCart) {


                const increaseQuantity = await cartModel.findOneAndUpdate({ _id: cartId, 'items.productId': productId }, { $inc: { 'items.$.quantity': Number(+1) } })
                console.log("increaseQuantity", increaseQuantity)                                                       //{ $inc: { 'items.$.quantity': Number(-1) } }

                const increaseTotalPrice = await cartModel.findOneAndUpdate({ _id: cartId, 'items.productId': productId }, { totalPrice: totalPriceInCart + productPrice }, { new: true })
                console.log("increaseTotalPrice", increaseTotalPrice)

                return res.status(201).send({ status: true, message: "success", data: increaseTotalPrice })

            } else {
                if (!checkProductInCart) {

                    const checkProductInCart2 = await cartModel.findOne({ _id: cartId, userId: userId })
                    ...
                    if (checkProductInCart2) {
                       ...
                       // const isItemAdded = cart.items.find(c => c['productId'] == productId)
                        ...
                        const addNewProduct = await cartModel.updateOne({ _id: cartId }, { $push: { items: [{productId:findProduct._id, quantity:quantityInCart}] } } )
...
const updateCart = async function (req, res) {
    try {
        ...
        if (!(removeProduct === 1 || removeProduct === 0)) {
            return res.status(400).send({ status: false, message: "removeProduct key must contain 0 or 1 " })
        }
        if (removeProduct === 1) {
            console.log("hy hy hy remove product")                                                                       // object of object ki nested key ko access krne kai lie $
            const variable = await cartModel.findOneAndUpdate({ _id: cartId, 'items.productId': productId }, { $inc: { 'items.$.quantity': Number(-1) } })
            console.log("variable", variable)
            console.log(priceInCart - singleProductPrice)
            let dec = await cartModel.findOneAndUpdate({ _id: cartId }, { totalPrice: priceInCart - singleProductPrice }, { new: true })
        ...
        if (removeProduct === 0) {
            ...
            let removePrdt = await cartModel.findOneAndUpdate({ _id: cartId }, { totalPrice: priceInCart - totalPrice }, { new: true })
            return res.status(200).send({ status: true, message: "this product remove successfully", data: removePrdt });
        }
    } catch (error) {
...
// if (!checkProductInCart) {
//     return res.status(404).send({ status: false, message: `this product is not present in cart` })
// }
// const increaseQuantity = await cartModel.findOneAndUpdate({ _id: cartId, 'items.productId': productId }, { $inc: { 'items.$.quantity': Number(+1) } })
// console.log("increaseQuantity", increaseQuantity)
// // const updatedPrice = productPrice * increaseQuantity
// const increaseTotalPrice = await cartModel.findOneAndUpdate({ _id: cartId, 'items.productId': productId }, { totalPrice: totalPriceInCart + productPrice })
// console.log("increaseTotalPrice", increaseTotalPrice)
// return res.status(201).send({ status: true, message: "success", data: increaseTotalPrice })
